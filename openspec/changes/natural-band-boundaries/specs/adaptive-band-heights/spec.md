...
**Reason**: With natural boundaries, `band.top` directly equals the boundary position which is inherently cumulative. No post-hoc recalculation needed.
**Migration**: Remove cumulative recalculation pass from `sliceIntoBands()`.

### Requirement: Band contentTop preserves grid-coordinate origin
**Reason**: With natural boundaries, `band.top` equals the grid-coordinate position. The dual-coordinate system (`top` vs `contentTop`) is unnecessary.
**Migration**: Remove `contentTop` field from `MasonryBandData`. Revert `renderBand` to use `band.top` for item positioning.
